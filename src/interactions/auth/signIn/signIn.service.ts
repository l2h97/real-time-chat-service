...
export class SignInService {
  ...
  async execute(payload: SignInPayloadDto): Promise<UserAuthResDto | UserResponseDto> {
    const {
      email, password,
    } = payload;
    ...
    const isMatchPassword = await passwordComparer(email, password, user.hashPassword);
    if (!isMatchPassword) {
    ...
    const {
      token, refreshToken,
    } = await this.userCreateTokenService.execute(user.id, user.email, user.userName);
