...
@Injectable()
export class RefreshTokenStrategy extends PassportStrategy(Strategy, 'jwt_refresh_token') {
  constructor(
    private configService: ConfigService<Configs, true>,
  ) {
    super({
      jwtFromRequest: ExtractJwt.fromExtractors([ExtractJwt.fromHeader('refresh_token')]),
